...
将数组容量扩大成 2 倍，然后将数组各个位置的元素重新寻找新的位置进行存放。因为数组的长度已经固定为了 2 次幂，所以元素重新查找位置时，并不需要重新进行 & 运算来寻找新的位置，只需要跟原数组的长度进行 & 运算，判断最高位的是否为 1，如果为 1，那么该元素就需要放到新的位置，这个新的位置为：原位置 + 原数组长度；如果为 0，则该元素在原位置不变。

如：

abcabcabc 这个字符串的哈希码为：0001 0010 0100 0011 1011 0011 1010 0010。假设原数组的容量为 16，二进制为 0001 0000，那么扩容后就为 32，二进制位 0010 0000。

我们计算一个元素的位置的公式：hashcode & (length - 1)

原位置：0001 0010 0100 0011 1011 0011 1010 0010 & 0000 1111 = 0010 = 2

新位置：0001 0010 0100 0011 1011 0011 1010 0010 & 0001 1111 = 0010 = 2



qwer 这个字符串的哈希码为：0011 0101 0010 1001 0101 0011

原位置：0011 0101 0010 1001 0101 0011 &  0000 1111 = 0011 = 3

新位置：0011 0101 0010 1001 0101 0011 &  0001 1111 =  0001 0011 = 16 + 3 = 19



因为容量扩大一倍之后，它在二进制上，相当于左移了一位，那么在计算位置的时候，就会多出一位的 1 出来，如果该位的 1 与 hashcode 对应位置上进行 & 操作，如果 hashcode 该位置为 0，那么它就跟原容量时相同，所以计算出来的位置也就不变，如果 hashcode 该位置为 1，那么它就会多出了一个原始容量的大小，所以这时候位置就等于：原容量大小 + 原位置。

![HashMap——resize](http://cdn.dengqiuying.cn/HashMap——resize.jpg)
